...
**GPTZero word budget** (~500k assumed; read `/v3/usage-stats` in hour 0, ask booth for a bump): interactive 120k · investigation 230k (7 yrs × 150 × ~200 words) · reserve 50k. Ledger enforces caps; `GPTZERO_MODE=replay` by default so dev burns nothing; results cached by sha256 and written back to ES docs as `gptzero.*`.
