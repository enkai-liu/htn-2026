...
// WebGL: the text alternative, the empty state, and the fallback to the 2D chart when WebGL is not available.
...
const Canvas = dynamic(() => import("./IslandMapCanvas"), {
  ssr: false,
  loading: () => <div className="absolute inset-0 flex items-center justify-center font-display text-[20px] italic text-mute">Charting the islands…</div>,
});
...
export function shapeIslands(graph: GraphState, layout: LayoutState): { islands: IslandDatum[]; links: SceneLink[] } {
  ...
  for (const id of layout.order) {
    ...
    const buildings = p.kind === "idea" ? 5 : p.kind === "prior" ? 0 : p.kind === "mutation" ? 1 : Math.min(5, Math.max(1, Math.round(n.val) - 1));
    islands.push({ id, label: n.label, tint, buildings, badges: n.badges, similarity: n.similarity ?? null, p });
  }
...
export function IslandMap({ graph, layout, fallback, ...rest }: IslandMapProps) {
  ...
  if (rest.ambient) return <div className="absolute inset-0" aria-hidden><Canvas islands={islands} links={links} layout={layout} onContextLost={() => setLost(true)} {...rest} /></div>;
  ...
  return (
    <figure className="absolute inset-0 m-0" aria-label={`Map of the idea-space: your idea at the centre, ${entities} prior-art projects placed by similarity, closer means more similar.`}>
      <Canvas islands={islands} links={links} layout={layout} onContextLost={() => setLost(true)} {...rest} />
