...
class LLMRouter:
    ...
    async def chat(self, *, role: str, messages: list[dict], model: str | None = None, temperature: float = 0.2,
        ...
        for name, client in self.providers:
            ...
            for attempt in range(2):  # one retry per provider on 429 / 5xx / timeout, then fall through
                ...
                try:
                    ...
                    usd, estimated = cost.estimate(wanted, tin, tout)
        ...
        raise LLMUnavailable(f"all providers failed for {wanted}: {type(last_exc).__name__}: {last_exc}")
