...
def test_release_gives_the_words_back(tmp_path):
    led = ledger_at(tmp_path)
    r = led.reserve("interactive", 800)
    with pytest.raises(BudgetExceeded):
        led.reserve("interactive", 400)
    led.release(r)
    assert led.remaining("interactive") == 1_000 and led.used("interactive") == 0
    led.release(r)  # idempotent
    led.commit(led.reserve("interactive", 400), actual_words=300)  # the API's own count wins when we have it
    assert led.used("interactive") == 300
...
def test_persists_across_instances_and_is_valid_json(tmp_path):
    ...
    b = ledger_at(tmp_path)  # e.g. a second process next to the running backend
...
def test_reset(tmp_path):
    ...
    assert led.used("interactive") == 0
    led.commit(led.reserve("interactive", 20))
    led.reset()
    assert led.used("interactive") == 0


def test_threads_never_overspend(tmp_path):
    led = WordLedger(tmp_path / "ledger.json", caps={"interactive": 8 * 25 * 10})
...
def test_two_handles_on_one_file_contend_safely(tmp_path):
    ...
    def worker() -> None:
        led = WordLedger(path, caps={"interactive": cap})
        for _ in range(40):
            try:
                led.commit(led.reserve("interactive", 5))
    ...
    [t.start() for t in threads]
    [t.join() for t in threads]
    final = WordLedger(path, caps={"interactive": cap})
...
async def test_safe_from_asyncio_tasks(tmp_path):
    led = WordLedger(tmp_path / "ledger.json", caps={"interactive": 500})
    refused = 0
...
def test_default_ledger_uses_settings_and_runs_dir():
    ...
    assert led.caps == {"interactive": s.gptzero_interactive_word_cap}
